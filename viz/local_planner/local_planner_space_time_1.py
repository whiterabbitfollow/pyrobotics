...
class DummySpaceTimeSpace:
    ...
    def is_collision_free_transition(self, state_src, state_dst, min_step_size):
        # if np.isclose(state_src, [1, 6]).all() and np.isclose(state_dst, [1, 7]).all():
        if state_src[-1] == 1 and state_src[0] >= 0:
            status = False
        else:
            status = True
        return status
...
path_pp = post_process_path_space_time_minimal_time(space, local_planner, goal_region, path, max_cnt_no_improvement=10)
